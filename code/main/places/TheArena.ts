...
///<reference path="../modules/TheArenaModule.ts"/>
...
class TheArena extends Place{
    ...
    private drawQuestLogo(questFolderName: string, x: number, y: number): void{
        // If the quests folder name given isn't null
        if(questFolderName != null){
            // If this quests doesn't exist
            if(TheArenaModule.getQuest(questFolderName) == null)
                console.log("Trying to draw the arena quests " + questFolderName + " which wasn't added to the arena module.");
            // Else, this quests exists
            else{
                // Draw the logo
    ...
    private update(): void{
        ...
        // Create your quests!
        this.renderArea.drawString("If you're a programmer and you want to create a quests,                     and                 !", 1, 15);
