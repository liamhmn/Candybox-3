...
class SecondHouse extends House{
    ...
    // Selected items (an items is selected when the players clicks on it)
    private selectedItemIndex: number = null;
    
    // Should we draw the intro speech ? (will be set to false as soon as we select an items)
    ...
    public willBeDisplayed(): void{
        // We need to update each time we're going to be displayed in case some items should not be sold anymore because of some stuff we were doing while this place wasn't displayed
        this.update();
    }
    ...
    private buySelectedItem(): void{
        ...
        if(this.getGame().getCandies().getCurrent() >= this.items[this.selectedItemIndex].getPrice()){
            ...
            // We tell the items that we just bought it
    ...
    private clickedOnItem(index: number): void{
        // Set the currently selected items index
    ...
    private drawItems(): void{
        for(var i = 0; i < this.items.length; i++){
            // If we can show this items
            if(this.items[i].canBeShown()){
                // We draw the items at the correct position
    ...
    private update(): void{
        ...
        // If there's a selected items, draw the stuff related to it (speech and possibly buying button)
