...
class MonkeyWizard extends QuestEntity{
    ...
    public update(): void{
        ...
        if(Math.abs(playerPos.x - this.getGlobalPosition().x) < 15){
            // If we're going left
            if(this.goingLeft){
                // If we're on the right of the quests
            ...
            // Else, if we're going right
                ...
                if(playerPos.x < this.getGlobalPosition().x){
                    // If we're on the left side of the quests
                ...
                // Else, the player is on the right and we're not too much on the left of the quests
