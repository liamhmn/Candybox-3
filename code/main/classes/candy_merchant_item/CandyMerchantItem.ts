...
class CandyMerchantItem{
    ...
    // The saving bool, used to remember if the items was bought or not
    ...
    // The database name of the speech the merchant should deliver when we click on the items
    ...
    // The class used both for the button to make the items clickable and the buying button
    ...
    // Public methods
    public buy(): void{
        // We tell the saving system that this items is bought
        Saving.saveBool(this.savingBool, true);
    }
    
    public canBeBought(): boolean{ // Return true if we can buy the items, else return false
    ...
    public canBeClicked(): boolean{ // Return true if you can click on the items, else return false
        // If it's already bought, we return false
        if(this.isBought())
            return false;
